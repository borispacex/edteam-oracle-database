...
SELECT TO_CHAR(TO_DATE('2000-06-10'), 'Day' ) FROM DUAL;
...
---- Crear un reporte de la tabla DOCTORS, que me permita ver los nombres, apellidos y la edad.
-- debe ser trabajado desde una FUNCTION
-- Campos a mostrar: nombres, apellidos, edad 
...
SELECT info.name, info.lastname FROM TABLE (get_report_doctors) info; 
...
SELECT v2.* FROM TABLE (get_report_doctors_v2) v2;

SELECT info.name, info.lastname, info.age FROM TABLE (get_report_doctors_v2) info WHERE info.age = 32;
